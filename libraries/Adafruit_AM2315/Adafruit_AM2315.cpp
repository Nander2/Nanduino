...
#define ErrorMaxCount	3
#define ReadMinInterval	2000 //2s
...
void Adafruit_AM2315::IncrementErrorCounter(void)
{
	ReadErrorCounter++;
	if(ReadErrorCounter >= ErrorMaxCount)
		begin();
}

boolean Adafruit_AM2315::readData(void) 
{
	if(millis() - LastReadMillis >= ReadMinInterval)
	{
		uint8_t reply[10];
...
double Adafruit_AM2315::readTemperature(void) 
{
	if (! readData()) return NAN;
	return temp;
}

double Adafruit_AM2315::readHumidity(void) 
{
	if (! readData()) return NAN;
	return humidity;
}
 ...
 */
bool Adafruit_AM2315::readTemperatureAndHumidity(double &t, double &h) 
{
